...
# Choose worker script based on job type
        if payload_json.get('search_type') == 'residue_sieve':
            ...
            worker_args = f"--job-file {payload_filename} --gpu-id {gpu_id}"
        else:
...
# Choose which script to run based on job type
            if job.search_type == 'residue_sieve':
                worker_script = "sieve_filter.py"
            else:
            ...
            cmd_str = (
                ...
                f"python -u {worker_script} --job-file {job_file} --gpu-id 0 {mining_flag}"
            ).strip()
...
#            self._aggregate_sieve_results(analysis_id, output_file)
    ...
    def _create_sieve_jobs(self, args):
        ...
        seeds_per_worker = total_seeds // len(workers)
        ...
        for i, worker in enumerate(workers):
            seed_start = base_seed_start + (i * seeds_per_worker)
...
def main():
    ...
    parser.add_argument('--prng-type', default='lcg32',
                        choices=['lcg32', 'xorshift32', 'pcg32', 'mt19937'],
    ...
    parser.add_argument('--skip', type=int, default=0, choices=range(17),
                        help='Skip/burn-in count (0-16)')
