...
class SSHConnectionPool:
    ...
    def cleanup_all(self, timeout=2.0):
        """Clean up all connections with timeout"""
        for hostname in list(self.connection_pools.keys()):
            try:
                # Try to acquire lock with timeout
                if self.pool_locks[hostname].acquire(timeout=timeout):
                    try:
                        while not self.connection_pools[hostname].empty():
                            try:
                                ssh = self.connection_pools[hostname].get_nowait()
                        ...
                        self.pool_locks[hostname].release()
                else:
                    # Timeout - skip this hostname
                    print(f"⚠️  Cleanup timeout for {hostname}, skipping...")
            except:
                pass
