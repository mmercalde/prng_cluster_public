...
=====================================
Fix v2: Extract IP from worker_id instead of DNS resolution.
worker_id format: "192_168_3_120_gpu0" → IP "192.168.3.120"
DNS-based approach failed because Zeus can't resolve rig-6600.

Replaces apply_s161_dashboard_hostname.py patches.

Apply:
    python3 apply_s161_dashboard_hostname_v2.py --dry-run
    python3 apply_s161_dashboard_hostname_v2.py
"""
import argparse, ast, shutil, sys
from pathlib import Path

TARGET = Path("persistent_worker_coordinator.py")
BACKUP = Path("persistent_worker_coordinator.py.pre_s161_dashboard_hostname_v2")

# Patch 1: Replace DNS-based hostname map with worker_id-based extraction
OLD_NODE_MAP = '''        # S161: build reverse map hostname→IP for TCP dashboard fix
...
NEW_NODE_MAP = '''        # S161 v2: no hostname map needed — extract IP from worker_id directly
        # worker_id format: "192_168_3_120_gpu0" → IP "192.168.3.120"
        # DNS resolution not used — Zeus cannot resolve rig-* hostnames'''

# Patch 2: Replace DNS resolution with worker_id parsing
...
PATCHES = [
    ("hostname map — worker_id approach", OLD_NODE_MAP, NEW_NODE_MAP),
    ("dashboard worker_id IP extraction",  OLD_DASH,     NEW_DASH),
]
