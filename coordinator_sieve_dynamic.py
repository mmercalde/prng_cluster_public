...
VERSION: 1.6.1 - Fault-Tolerance Hotfix (No-CUDA in Coordinator, PG-Kill, Localhost Throttle)
CHANGES:
...
ISSUE RESOLVED: Zeus systemd segfaults from resource exhaustion during 156 concurrent jobs
FIX: Coordinator no longer touches CUDA APIs; local jobs are killed via process-group on timeout
NEW FEATURE: Automatic fault tolerance - transparently recovers from connection failures
and resumes incomplete jobs without requiring manual intervention from unified_system_working.py
"""
...
class MultiGPUCoordinator:
    ...
    def _build_sh_safe_cmd(self, node: WorkerNode, payload_filename: str, payload_json: dict,
        ...
        py = node.python_env
# Choose worker script based on job type
        ...
        # Add GPU ID and mining mode parameters
        mining_flag = "--mining-mode" if payload_json.get('mining_mode', False) else ""
# Command body using conditional worker_args
    ...
    def _preflight_node(self, node: WorkerNode) -> Tuple[bool, list, list]:
        ...
        if self.is_localhost(node.hostname):
            ...
            try:
                ...
                print(f"🔍 SSH Debug - {node.hostname}: {type(e).__name__}: {str(e)}")
    ...
    def test_connectivity(self) -> bool:
        ...
        for node in self.nodes:
            ...
            print(f"✅ {node.hostname}: {len(node_workers)} GPUs available ({capacity:,} seeds/GPU)")
            ok, warns, errs = self._preflight_node(node)
            for w in warns:
                print(f" ⚠️ {node.hostname}: {w}")
            for e in errs:
            ...
            for worker in sorted_workers:
                ...
                if seeds_to_assign > 0:
                    ...
                    job = JobSpec(
                        ...
                        search_type="draw_match" if (hasattr(args, "draw_match") and args.draw_match is not None) else "standard",
    ...
    def execute_local_job(self, job: JobSpec, worker: GPUWorker) -> JobResult:
        ...
        try:
            node = worker.node
            # Build job payload + pass target_file
# Build job payload based on job type
            print(f"🔍 DEBUG: job_id={job.job_id}, prng_type={job.prng_type}, search_type={job.search_type}", file=sys.stderr)
            ...
            # Use bash so 'source' works; map GPU using CUDA_VISIBLE_DEVICES and pass --gpu-id 0
            # Choose worker script and command format based on job type
            if job.search_type == 'residue_sieve':
    ...
    def execute_remote_job(self, job: JobSpec, worker: GPUWorker) -> JobResult:
        ...
        try:
            node = worker.node
            # Create job specification (+ target_file passthrough)
# Create job specification based on job type
            if job.search_type == 'residue_sieve':
                # Sieve requires completely different job structure
                seed_start = job.seeds[0] if isinstance(job.seeds, list) and len(job.seeds) == 2 else 0
                seed_end = job.seeds[1] if isinstance(job.seeds, list) and len(job.seeds) == 2 else 100000
            ...
            if hasattr(self, '_debug_failed_nodes') and node.hostname in self._debug_failed_nodes:
                print(f"🔍 Debug command for {node.hostname}:")
            ...
            output, errout = stdout.read().decode(), stderr.read().decode()
        ...
        def dynamic_worker_loop(worker):
            ...
            while True:
                ...
                try:
                    ...
                    print(f"❌ {gpu_info} | Job execution failed: {str(job_error)}")
                ...
                if result.success:
                    ...
                    print(f"✅ Dynamic | {gpu_info} | {len(job.seeds):,} seeds | {result.runtime:.1f}s | {seeds_per_sec:.0f} seeds/sec | [{scaling_factor:.1f}x scaling]")
                else:
                    print(f"❌ Dynamic | {worker.node.gpu_type}@{worker.node.hostname} | FAILED: {result.error[:100] if result.error else 'Unknown error'}")
        ...
        with ThreadPoolExecutor(max_workers=len(workers)) as executor:
            ...
            while True:
                ...
                if current_time - last_update > 5: # Every 5 seconds
                    ...
                    print(f"🔄 Dynamic Progress: {percentage:.1f}% complete | "
            ...
            for i, future in enumerate(futures):
                try:
                    ...
                    print(f"⏳ Waiting for {worker_info} to complete...")
                    worker_results = future.result(timeout=self.job_timeout * 2)
                    print(f"✅ {worker_info} completed {len(worker_results)} jobs") # 2x job timeout for safety
                    all_results.extend(worker_results)
                except Exception as e:
                    worker = workers[i] if i < len(workers) else None
                    worker_info = f"{worker.node.gpu_type}@{worker.node.hostname}" if worker else f"Worker {i}"
                    print(f"❌ {worker_info} | Worker failed with error: {str(e)}")
    ...
    def execute_truly_parallel_dynamic(self, target_file: str, output_file: str, args,
        ...
        print("🚀 True Parallel Dynamic Distribution Mode")
        ...
        while current_seed < 1000 + total_seeds:
            seeds_this_chunk = min(base_chunk_size, (1000 + total_seeds) - current_seed)
            seed_list = list(range(current_seed, current_seed + seeds_this_chunk))
            # Build job_spec based on analysis method
        ...
        def worker_loop(worker):
            ...
            while True:
                try:
                    ...
                    # Convert to appropriate job format
                    ...
                    print(f"❌ Worker error on {worker.node.hostname}: {str(e)}")
                    break
        ...
        while completed_chunks < total_chunks:
            ...
            if completed_chunks > 0:
                ...
                print(f"🔄 Parallel Progress: {progress_pct:.1f}% | {completed_chunks}/{total_chunks} chunks completed")
        ...
        with ThreadPoolExecutor(max_workers=min(len(jobs), self.max_concurrent)) as executor:
            ...
            for future in as_completed(future_to_job):
                ...
                try:
                    ...
                    if result.success:
                        print(f"✅ Dataset chunk | {worker.node.hostname} | "
                              f"{len(job.dataset_values)} values | {result.runtime:.1f}s")
                    else:
                        print(f"❌ Dataset chunk | {worker.node.hostname} | FAILED")
                except Exception as e:
                    print(f"❌ Job error: {e}")
        # Compile results
    ...
    def execute_distributed_analysis(self, target_file: str, output_file: str,
        ...
        # ALL methods now use dynamic distribution (including sieve)
        use_dynamic = True
        ...
        if use_dynamic and use_parallel_dynamic:
            ...
            print("⚙️ Using Traditional Static Distribution Mode with GPU Optimization")
            # Continue with existing static method below...
        print("Multi-GPU Distributed PRNG Analysis with Automatic Fault Tolerance")
        print(f"Target: {total_seeds:,} total seeds, {samples:,} samples each")
        # remember target file for job payloads
        self.current_target_file = target_file
        ...
        # Check for previous analysis and decide action
        ...
        if existing_progress:
            ...
            if action == 'restart':
                ...
                if removed:
                    print("🧹 Old progress file deleted. Starting a fresh analysis...")
                else:
                    print("⚠️ Could not delete old progress file (will overwrite as we go). Starting fresh.")
                existing_progress = None
            else:
                print("🔁 Resuming from saved progress...")
                progress_state = existing_progress
                # Get remaining jobs to execute
                remaining_jobs: List[Tuple[JobSpec, GPUWorker]] = []
                with self._progress_lock:
                    for job_data in progress_state.pending_jobs:
                        job, worker = self.deserialize_job_assignment(job_data)
                        # ensure attempt is aligned with retry_count
                        job.attempt = progress_state.retry_count.get(job.job_id, 0)
                        remaining_jobs.append((job, worker))
                # Add retry jobs for failed jobs that haven't exceeded retry limit (with possible migration)
        ...
        if not existing_progress:
            ...
            prng_types = ["mt", "xorshift", "lcg"]
            mapping_types = ["mod", "scale"]
            print(f'DEBUG: hasattr(args, method): {hasattr(args, "method")}')
            if hasattr(args, 'method'):
                print(f'DEBUG: args.method value: {repr(args.method)}')
                print(f'DEBUG: condition check: {args.method == "residue_sieve"}')
            else:
                print('DEBUG: args has no method attribute')
                        # Handle residue sieve method
            if hasattr(args, "method") and args.method == "residue_sieve":
        ...
        # Execute remaining jobs with automatic progress tracking
        if remaining_jobs:
            ...
            with ThreadPoolExecutor(max_workers=max_concurrent) as executor:
                ...
                for job, worker in remaining_jobs:
                    future = executor.submit(self.execute_gpu_job, job, worker)
                    future_to_job[future] = (job, worker)
                # Collect results as they complete with automatic progress tracking
                for future in as_completed(future_to_job):
                    job, worker = future_to_job[future]
                    try:
                        result = future.result()
                        ...
                        if result.success:
                            print(f"✅ {worker.node.hostname} GPU{worker.gpu_id} | {job.prng_type}-{job.mapping_type} | {result.runtime:.1f}s | {len(job.seeds)} seeds")
                        else:
                            with self._progress_lock:
                                retry_count = progress_state.retry_count.get(job.job_id, 0)
                            max_retries = self.recovery_manager.max_retries
                            if retry_count < max_retries:
                                print(f"⚠️ {worker.node.hostname} GPU{worker.gpu_id} | {job.prng_type}-{job.mapping_type} | {result.runtime:.1f}s | Failed (will retry {retry_count}/{max_retries})")
                            else:
                                print(f"❌ {worker.node.hostname} GPU{worker.gpu_id} | {job.prng_type}-{job.mapping_type} | {result.runtime:.1f}s | Failed (max retries exceeded)")
                        ...
                        if current_time - last_progress_save >= self.progress_save_interval:
                            ...
                            print(f"💾 Progress saved - Completed: {completed}/{total} ({progress_pct:.1f}%) | Failed: {failed}")
                        # Coordinator-only memory cleanup (no CUDA calls)
                        with self._progress_lock:
                        ...
                        print(f"❌ ERROR: {worker.node.hostname} GPU{worker.gpu_id} | {error_msg}")
            ...
            with self._progress_lock:
                self.recovery_manager.save_progress(progress_state)
        # Check if we need to retry any failed jobs
        retry_jobs = self.get_retry_jobs(progress_state)
        if retry_jobs:
            # Calculate backoff from the highest attempt in this batch
            with self._progress_lock:
                max_attempt = 0
            ...
            time.sleep(delay)
            # Recursively call with updated progress (automatic retry)
        ...
        # Get permanently failed jobs (exceeded max retries)
        with self._progress_lock:
            permanently_failed = {
...
""".rstrip())
# Aggregate sieve results if this was a sieve analysis
#        if hasattr(self, '_sieve_config'):
#            self._aggregate_sieve_results(analysis_id, output_file)
        # Clean up progress file if analysis completed successfully
        if len(permanently_failed) == 0:
            self.recovery_manager.cleanup_progress(analysis_id)
            print("✅ Analysis completed successfully - progress file cleaned up")
        else:
            print(f"⚠️ {len(permanently_failed)} jobs failed permanently - progress file retained for investigation")
        return final_results
    ...
    def _create_sieve_jobs(self, args):
        ...
        num_jobs = min(total_seeds, len(workers))
        ...
        self._sieve_config = {
            ...
            'prng_families': [args.prng_type.replace('_reverse', '').replace('_hybrid_reverse', '')] if hasattr(args, 'prng_type') else self.sieve_defaults.get('prng_families', ['mt19937']),
        ...
        if use_hybrid:
            if HYBRID_AVAILABLE:
                ...
                try:
                    ...
                    self._sieve_config['strategies'] = []
                print(f"🔬 Hybrid mode enabled with {len(self._sieve_config.get('strategies', []))} strategies")
            else:
    ...
    def _create_reverse_sieve_jobs(self, args, candidate_seeds: list):
        ...
        print(f"✅ Created {len(jobs)} reverse sieve jobs", file=sys.stderr)
        return jobs
...
def main():
    ...
    parser.add_argument('target_file', help='Target data file (e.g., daily3.json)')
    ...
    parser.add_argument('--max-per-node', type=int, default=4,
    ...
    parser.add_argument('--prng-type', default='lcg32',
         ...
         'lcg32_reverse', 'lcg32_hybrid_reverse',
            'xorshift32_reverse', 'xorshift32_hybrid_reverse', 'xorshift64_reverse', 'xorshift64_hybrid_reverse', 'xorshift128_reverse', 'xorshift128_hybrid_reverse', 'pcg32_reverse', 'pcg32_hybrid_reverse', 'java_lcg_reverse', 'java_lcg_hybrid_reverse', 'minstd_reverse', 'minstd_hybrid_reverse', 'philox4x32_reverse', 'philox4x32_hybrid_reverse', 'minstd_reverse', 'minstd_hybrid_reverse',
    ...
    if not args.output:
        ...
        args.output = f"results/multi_gpu_analysis_{timestamp}.json"
    try:
        ...
        if args.test_only:
            ...
            if coordinator.test_connectivity():
                print("✅ All GPU workers accessible")
                return 0
            else:
