#!/bin/bash
# Test one of the failing seeds directly on the remote node via coordinator
cat > /tmp/test_remote_seed.py << 'PYEOF'
import sys, json
...
ssh michael@192.168.3.120 "cd /home/michael/distributed_prng_analysis && source ~/rocm_env/bin/activate && python3 /tmp/test_remote_seed.py"
