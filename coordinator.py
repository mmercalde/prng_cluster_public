...
VERSION: 1.7.4.1 - Per-Node Concurrency Limits (Script Jobs Only)
CHANGES:
...
- v1.7.3: Route script jobs through distributed_worker.py (NOT direct SSH execution);
         Uses same Python→SSH→distributed_worker pattern as seed jobs; Enables GPU
         initialization and ROCm environment setup for ML training jobs
- v1.7.4: REMOVED - Had structural bug (seed code unreachable after script continue)
- v1.7.4.1: Per-node concurrency limits for script jobs (lines 89-105, 821-856, 1218-1372);
         Prevents CPU overload on weak mining rig CPUs (4-core i3, 6-core i5);
         Added thread-safe capacity tracking with configurable limits per node;
         Seed jobs bypass all limits (unlimited); Script jobs throttled (2/4/6 concurrent);
         Fixed worker_loop structure: unified try/finally for both job types ensures
         decrement_node_jobs() and task_done() ALWAYS called; Fixed timeout bug with
         capacity_timeout flag and continue; CRITICAL FIXES for seed jobs: (1) Removed
         payload parameter from seed JobSpec construction (lines 1297, 1311) - seed jobs
         don't have payload; (2) Fixed all payload checks to test for None first at lines
         552, 749, 1729: changed "if 'script' in job.payload:" to "if job.payload and
...
class MultiGPUCoordinator:
    def __init__(self, config_file: str = "distributed_config.json",
        ...
        # v1.7.4.1 - Per-node concurrency limits for script jobs
        ...
        # v1.7.4.1 - Per-node concurrency limits initialized in load_configuration()
    ...
    def execute_local_job(self, job: JobSpec, worker: GPUWorker) -> JobResult:
        ...
        try:
            ...
            # --- MODIFIED: Handle 'script' based jobs ---
            if job.payload and 'script' in job.payload:
            ...
            if 'script' in job_data:
                # Route through distributed_worker.py for ThreadPool execution
    ...
    # v1.7.4.1 - Per-Node Concurrency Management Methods
    ...
    def _increment_node_jobs(self, hostname: str, analysis_type: str):
        """
        Increment active job counter for a node (thread-safe).
        Only tracks script jobs; seed jobs are not counted.
        """
        if analysis_type != 'script':
            return  # No-op for seed jobs
    ...
    def _decrement_node_jobs(self, hostname: str, analysis_type: str):
        """
        Decrement active job counter for a node (thread-safe).
        Only tracks script jobs; seed jobs are not counted.
        """
        if analysis_type != 'script':
            return  # No-op for seed jobs
    ...
    def execute_truly_parallel_dynamic(self, target_file: str, output_file: str, args,
        ...
        def worker_loop(worker):
            ...
            while True:
                # Step 1: Get job from queue (with Empty exception handling)
                try:
                    ...
                    print(f"❌ Worker error getting job on {hostname}: {str(e)}")
                    break

                # Step 2: Determine job type
                analysis_type = job_spec.get('analysis_type', 'unknown')

                # Step 3: Wait for node capacity (only for script jobs)
                wait_start = time.time()
                capacity_timeout = False
                while not self._can_node_accept_job(hostname, analysis_type):
                    ...
                    if time.time() - wait_start > 300:  # 5 minute max wait
                        ...
                        capacity_timeout = True
                        break

                # Skip job if we timed out waiting for capacity
                if capacity_timeout:
                    continue
                ...
                try:
                    ...
                    if analysis_type == 'script':
                        ...
                        # Handle seed-based jobs (correlation, sieve, etc.)
                        ...
                        if job_spec.get('search_type') == 'residue_sieve':
                            ...
                            job = JobSpec(
                                ...
                                lmax=job_spec.get('lmax', 32),
                                grid_size=job_spec.get('grid_size', 16),
                                prng_type=job_spec.get('prng_type', 'java'),
                                mapping_type=job_spec.get('mapping_type', 'modulo'),
                                search_type='residue_sieve',
                                mining_mode=False,
                                target_draw=None,
                                analysis_type='residue_sieve',
                                attempt=0
                            )
                        else:
                            # Convert to JobSpec object for correlation/other jobs
                            job = JobSpec(
                                job_id=job_spec.get('job_id', 'unknown'),
                                ...
                                mining_mode=False,
                                target_draw=None,
                                analysis_type=job_spec.get('analysis_type', 'correlation'),
                                attempt=0
                            )
                        # Execute the job
                        result = self.execute_gpu_job(job, worker)
                    ...
                    print(f"❌ Worker error on {hostname} processing job: {str(e)}")
                    import traceback
                    traceback.print_exc()

                finally:
                    # ALWAYS decrement counter and mark task done
