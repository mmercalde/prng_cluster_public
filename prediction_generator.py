...
Date: January 2, 2026
Version: 6.1 (Signal Gate Fix)

v6.1 Changes:
- CRITICAL FIX: Check signal_quality BEFORE loading model
- Handles degenerate sidecar (checkpoint_path: null)
- Exit code 3 for PREDICTION_SKIPPED
- _signal_gate_blocked flag for early exit
...
EXIT CODES:
  0 - Success (predictions generated)
  1 - Error (exception)
  2 - No model found  
  3 - Signal gate blocked (PREDICTION_SKIPPED)
"""
...
class PredictionGenerator:
    ...
    def __init__(self, config: PredictionConfig, logger: logging.Logger):
        ...
        self.logger.info("PredictionGenerator initialized (v6.1)")
        ...
        # Model loading (Step 6 v6.1 - Signal Gate Fix)
        self.model = None
        ...
        if MULTI_MODEL_AVAILABLE:
            ...
            sidecar_path = Path(config.models_dir) / "best_model.meta.json"
            
            if not sidecar_path.exists():
                self.logger.error(f"Sidecar not found: {sidecar_path}")
                raise FileNotFoundError(f"Required sidecar missing: {sidecar_path}")
            
            with open(sidecar_path) as f:
                self.model_meta = json.load(f)
            
            self.logger.info(f"  Sidecar loaded: {sidecar_path}")
            
            # === STEP 2a: Extract parent_run_id FIRST (before gate check) ===
            ...
            # === STEP 2b: Check signal gate BEFORE model load ===
            ...
            if sq:
                self.logger.info(f"  Signal quality: {signal_status}")
                self.logger.info(f"  Prediction allowed: {prediction_allowed}")
            else:
                self.logger.warning("  No signal_quality in sidecar (legacy model)")
            
            # === STEP 3: Gate if signal is degenerate ===
            if not prediction_allowed:
                self.logger.warning("=" * 60)
                self.logger.warning("SIGNAL QUALITY GATE - EARLY BLOCK")
                self.logger.warning("=" * 60)
                self.logger.warning(f"  Status: {signal_status}")
                self.logger.warning(f"  Model was not trained (degenerate signal)")
                self.logger.warning(f"  checkpoint_path: {self.model_meta.get('checkpoint_path')}")
                self.logger.warning("=" * 60)
                self.logger.warning("Skipping model load - no valid model exists")
                self.logger.warning("WATCHER AGENT will decide recovery action")
                self.logger.warning("=" * 60)
                
                # Store gate status for generate_predictions to handle
                self._signal_gate_blocked = True
                self._signal_gate_result = {
                    "status": "PREDICTION_SKIPPED",
                    "reason": signal_status,
                    "signal_quality": sq,
                    "data_context": self.model_meta.get("data_context", {}),
                    "explanation": "Model training skipped due to degenerate signal",
                    "recovery_authority": "WATCHER_AGENT",
                    "timestamp": datetime.now().isoformat()
                }
                return  # Exit __init__ without loading model
            
            # === STEP 4: Signal OK - now load model ===
            checkpoint_path = self.model_meta.get("checkpoint_path")
            if checkpoint_path is None:
                raise ValueError("checkpoint_path is null but prediction_allowed is True - sidecar inconsistent")
            
            try:
                self.model, _ = load_model_from_sidecar(
    ...
    def generate_predictions(
        ...
        v6.1 Compliance:
        - Early signal gate check (blocked in __init__)
        ...
        # =====================================================================
        # v6.1: EARLY SIGNAL GATE CHECK (blocked during __init__)
        # =====================================================================
        if self._signal_gate_blocked:
            self.logger.warning("=" * 60)
            self.logger.warning("PREDICTION_SKIPPED - Signal gate blocked during init")
            self.logger.warning("=" * 60)
            
            if self.config.save_predictions:
                self._save_predictions(self._signal_gate_result)
            return self._signal_gate_result
        # =====================================================================
...
def main():
    ...
    logger = setup_logging(config)
    logger.info("=== Prediction Generator v6.1 Started ===")
    ...
    # v6.1: Handle signal gate exit code
    if result.get('status') in ['prediction_skipped', 'PREDICTION_SKIPPED']:
        logger.warning("=" * 60)
        logger.warning("PREDICTION_SKIPPED")
        logger.warning("=" * 60)
        logger.warning(f"Reason: {result.get('reason')}")
        logger.warning(f"Signal: {result.get('signal_quality', {}).get('signal_status', '?')}")
        logger.warning(f"Fingerprint: {result.get('data_context', {}).get('fingerprint_hash', '?')}")
        logger.warning("=" * 60)
        logger.warning("WATCHER AGENT should consume this and decide recovery")
        logger.warning("=" * 60)
        return 3  # Signal gate exit code
    else:
