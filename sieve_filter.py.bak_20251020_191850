        ...
        with self.device:
            ...
            for chunk_start in range(seed_start, seed_end, chunk_size):
                ...
                if prng_family == 'xorshift32':
                    ...
                    kernel_args.append(cp.int32(0))  # dummy1
                    kernel_args.append(cp.int32(0))  # dummy2
                    kernel_args.append(cp.int32(0))  # dummy3
# mt19937 and xorshift64 have no extra params before offset
    ...
    def run_hybrid_sieve(
        ...
        # Check if this PRNG supports hybrid mode (variable skip patterns)
        ...
        with self.device:
            ...
            # Handle strategies as dicts or objects
            ...
            for chunk_start in range(seed_start, seed_end, chunk_size):
                ...
                # Allocate arrays
                # Determine dtype based on seed_type
                seed_type = config.get("seed_type", "uint32")
                dtype = cp.uint64 if seed_type == "uint64" else cp.uint32
                seeds_gpu = cp.arange(chunk_start, chunk_end, dtype=dtype)
                survivors_gpu = cp.zeros(n_seeds, dtype=dtype)
                ...
                kernel_args = [
                    ...
                    # Add PRNG-specific params based on family
                    # Add PRNG-specific params based on family
                    *(
                        ...
                        []  # xorshift64, mt19937 have no extra params
                    ),
                    cp.int32(offset)
                ]

                # DEBUG: Print kernel_args
                print(f'DEBUG: Calling kernel with {len(kernel_args)} args', file=sys.stderr)
                for idx, arg in enumerate(kernel_args):
                    if hasattr(arg, 'shape'):
                        print(f'  {idx}: {type(arg).__name__} shape={arg.shape}', file=sys.stderr)
                    else:
                        print(f'  {idx}: {type(arg).__name__} = {arg}', file=sys.stderr)
                kernel((blocks,), (threads_per_block,), tuple(kernel_args))
        ...
        for family_spec in prng_families:
            ...
            if use_hybrid and supports_hybrid:
                ...
                if use_hybrid and strategies:
                    # Check if this is a single-phase hybrid (already has _hybrid suffix)
                    # vs two-phase hybrid (needs Phase 1 + Phase 2)
                    is_single_phase = ('_hybrid' in family_name)
                    
                    if is_single_phase:
                        # Single-phase: Direct hybrid sieve (e.g., xorshift32_hybrid)
                        print(f"Testing {family_name} in SINGLE-PHASE HYBRID mode...", file=sys.stderr)
                        ...
                        print(f"  Found {len(survivors)} survivors ({phase_duration:.1f}ms)", file=sys.stderr)
                        ...
                        result.update({
                            ...
                            'single_phase': {
                                ...
                                'duration_ms': round(phase_duration, 2),
                                'strategies_tested': len(strategies)
                            }
                        })
                        
                    elif family_config.get('multi_strategy', False):
                        # Two-phase: mt19937 -> mt19937_hybrid
