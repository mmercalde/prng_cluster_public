...
- ✅ **Skip Mode Metadata**: All survivors tagged with `skip_mode`, `prng_type`, `prng_base`
- ✅ **Enhanced ML Features**: 64 features (was 56), includes skip mode analysis
- ✅ **PRNG Flexibility**: Support for multiple PRNG types (java_lcg, mt19937, xorshift32, etc.)
- ✅ **Skip Mode Distribution**: Automatic reporting of constant vs variable survivor counts
...
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│  STEP 1: WINDOW OPTIMIZER (26-GPU Distributed)                  │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  window_optimizer.py                                      │  │
│  │  ├─ Bayesian Optimization (Optuna TPE)                    │  │
│  │  ├─ Calls window_optimizer_integration_final.py           │  │
│  │  └─ Generates optimal window parameters                   │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  window_optimizer_integration_final.py                    │  │
│  │  ├─ If test_both_modes=True:                              │  │
│  │  │  ├─ Forward sieve (constant: java_lcg)                 │  │
│  │  │  ├─ Reverse sieve (constant: java_lcg)                 │  │
│  │  │  ├─ Forward sieve (variable: java_lcg_hybrid)          │  │
│  │  │  └─ Reverse sieve (variable: java_lcg_hybrid)          │  │
│  │  ├─ Else (constant only):                                 │  │
│  │  │  ├─ Forward sieve (constant)                           │  │
│  │  │  └─ Reverse sieve (constant)                           │  │
│  │  └─ Tags survivors with skip_mode metadata                │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  MultiGPUCoordinator (coordinator.py)                     │  │
│  │  ├─ Distributes sieves across 26 GPUs                     │  │
│  │  ├─ Each GPU runs: sieve_filter.py                        │  │
│  │  └─ Aggregates results from all GPUs                      │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  📁 OUTPUTS:                                                     │
│     ├─ bidirectional_survivors.json (with skip_mode metadata)   │
│     ├─ forward_survivors.json                                   │
│     ├─ reverse_survivors.json                                   │
│     ├─ optimal_window_config.json                               │
│     ├─ train_history.json (80% of lottery data)                 │
│     └─ holdout_history.json (20% of lottery data)               │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│  STEP 2.5: SCORER META-OPTIMIZER (26-GPU Distributed) - PULL    │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  HEAD NODE (zeus): generate_scorer_jobs.py               │  │
│  │  ├─ Creates LOCAL Optuna study (sqlite on zeus)           │  │
│  │  ├─ Pre-samples N parameter sets via Optuna               │  │
│  │  ├─ Generates scorer_jobs.json (script-based format)      │  │
│  │  └─ Copies input data to all remote nodes via SCP         │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  HEAD NODE (zeus): MultiGPUCoordinator                    │  │
│  │  ├─ Reads scorer_jobs.json (script-based jobs)            │  │
│  │  ├─ Distributes jobs across 26 GPUs (local + remote)      │  │
│  │  ├─ Executes via SSH: scorer_trial_worker.py + args       │  │
│  │  └─ Jobs run independently on each node                   │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  WORKER NODES: scorer_trial_worker.py (on each GPU)       │  │
│  │  ├─ Receives: survivors, history, trial params as args    │  │
│  │  ├─ Loads data from LOCAL filesystem (pre-copied)         │  │
│  │  ├─ Tests one parameter configuration                     │  │
│  │  ├─ Computes accuracy score                               │  │
│  │  ├─ Writes result to LOCAL JSON file:                     │  │
│  │  │  ~/distributed_prng_analysis/scorer_trial_results/     │  │
│  │  │  trial_XXXX.json                                       │  │
│  │  └─ Does NOT access Optuna database (file-based only)     │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  HEAD NODE (zeus): collect_scorer_results() - PULL        │  │
│  │  ├─ SSH to each remote node                               │  │
│  │  ├─ SCP all trial_*.json files back to zeus               │  │
│  │  ├─ Delete remote files after successful transfer         │  │
│  │  ├─ Aggregate all results into single JSON                │  │
│  │  └─ Report results back to LOCAL Optuna study             │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  HEAD NODE (zeus): Optuna Study Analysis                  │  │
│  │  ├─ Find best trial from LOCAL Optuna database            │  │
│  │  ├─ Extract best parameters                               │  │
│  │  └─ Save to optimal_scorer_config.json                    │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  📁 OUTPUTS:                                                     │
│     ├─ optimal_scorer_config.json (best parameters)             │
│     ├─ aggregated_scorer_results.json (all trial results)       │
│     └─ optuna_studies/scorer_meta_opt_*.db (Optuna study)       │
│                                                                  │
│  🔑 KEY ARCHITECTURE NOTES:                                      │
│     ✅ NO shared filesystem required                            │
│     ✅ NO NFS/network storage dependencies                      │
│     ✅ Optuna DB only on head node (zeus)                       │
│     ✅ Workers write local JSON, coordinator pulls via SSH      │
│     ✅ Automatic cleanup of remote result files                 │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
│  │  ├─ Splits survivors across GPUs                          │  │
│  │  └─ Generates scoring_jobs.json                           │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  MultiGPUCoordinator                                      │  │
│  │  ├─ Distributes scoring jobs across 26 GPUs               │  │
│  │  └─ Each GPU runs: survivor_scorer.py                     │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  survivor_scorer.py (on each GPU)                         │  │
│  │  ├─ extract_ml_features() - NEW: 64 features              │  │
│  │  │  ├─ Original 56 features                               │  │
│  │  │  └─ NEW: 8 skip_mode features                          │  │
│  │  │     ├─ skip_mode_variable (binary)                     │  │
│  │  │     ├─ prng_* one-hot encoding (5 features)            │  │
│  │  │     └─ Interaction features (2 features)               │  │
│  │  ├─ score_survivor() - Calculates quality score           │  │
│  │  └─ Saves scored survivors                                │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  📁 OUTPUT: survivors_with_scores.json                           │
│             (All survivors with 64-feature vectors + scores)     │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
│  │  ├─ Analyzes survivor distribution                        │  │
│  │  ├─ Analyzes lottery data patterns                        │  │
│  │  ├─ Derives optimal NN architecture                       │  │
│  │  │  ├─ Number of layers                                   │  │
...
│  │  └─ Trains ReinforcementEngine models                     │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  ReinforcementEngine (reinforcement_engine.py)            │  │
│  │  ├─ Neural network with adaptive architecture             │  │
│  │  ├─ Trains on 64-feature vectors                          │  │
│  │  ├─ Learns skip_mode importance                           │  │
│  │  └─ Generates quality predictions                         │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  📁 OUTPUT: models/anti_overfit/best_model.pth                   │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
...
│  │  Tests on holdout_history.json                            │  │
│  │  Generates predictions for new seeds                      │  │
│  │  Reports accuracy metrics                                 │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  📊 OUTPUT: Predictions printed to console                       │
...
1. generate_scorer_jobs.py runs on zeus:
   ├─ Creates LOCAL Optuna study:
   │  sqlite:///./optuna_studies/scorer_meta_opt_TIMESTAMP.db
   ├─ Pre-samples N trials (e.g., 100 trials)
   │  Each trial = unique parameter combination:
   │  {
   │    "residue_mod_1": 15,
   │    "residue_mod_2": 67,
   ...
   │    "learning_rate": 0.001,
   │    ... (8-10 parameters total)
   │  }
   ├─ Creates scorer_jobs.json in script-based format:
   │  [
   │    {
   │      "job_id": "scorer_trial_0",
   │      "script": "scorer_trial_worker.py",
   │      "args": [
   │        "/path/to/bidirectional_survivors.json",
   │        "/path/to/train_history.json",
   │        "/path/to/holdout_history.json",
   │        "0",  ← trial number
   │        "{...params...}",  ← JSON string of parameters
   │        "--optuna-trial-number", "0"
   │      ],
   │      "expected_output": "scorer_trial_results/trial_0000.json"
   │    },
   │    ... (99 more jobs)
   │  ]
   └─ Copies input data to remote nodes:
...
┌────────────────────────────────────────────────────────────────┐
│ PHASE 2: JOB EXECUTION (distributed across 26 GPUs)            │
└────────────────────────────────────────────────────────────────┘

2. coordinator.py distributes jobs:
   ├─ Reads scorer_jobs.json
   ├─ Identifies 26 available GPUs:
   │  ├─ zeus GPU0, GPU1 (2 local GPUs)
   │  ├─ 192.168.3.120 GPU0-11 (12 AMD GPUs)
   │  └─ 192.168.3.154 GPU0-11 (12 AMD GPUs)
   ├─ Assigns jobs round-robin across GPUs
   └─ For each job, executes via SSH:
...
3. scorer_trial_worker.py (on each GPU):
   ...
   │  ├─ bidirectional_survivors.json (14,271 survivors)
   │  ├─ train_history.json (80% of lottery data)
   │  └─ holdout_history.json (20% holdout set)
   ├─ Initializes SurvivorScorer with trial parameters
   ├─ Runs scoring algorithm:
   │  ├─ Extracts ML features from survivors
   │  ├─ Trains neural network
   │  ├─ Evaluates on holdout set
   │  └─ Computes accuracy metric
   ├─ Writes result to LOCAL JSON file:
   │  ~/distributed_prng_analysis/scorer_trial_results/trial_0000.json
   │  {
   │    "trial_number": 0,
   ...
   │  }
   └─ Exits (does NOT touch Optuna database)

┌────────────────────────────────────────────────────────────────┐
│ PHASE 3: RESULT COLLECTION (coordinator pulls from all nodes)  │
└────────────────────────────────────────────────────────────────┘

4. coordinator.collect_scorer_results():
   ├─ For localhost (zeus):
   │  ├─ Reads ~/distributed_prng_analysis/scorer_trial_results/*.json
   │  ├─ Loads each JSON file into memory
   │  └─ Deletes local files after reading
   │
   ├─ For each remote node (192.168.3.120, 192.168.3.154):
   │  ├─ SSH to node
   │  ├─ Lists all trial_*.json files:
   │  │  ssh 192.168.3.120 "ls ~/distributed_prng_analysis/scorer_trial_results/trial_*.json"
   │  ├─ SCP each file back to zeus:
   │  │  scp 192.168.3.120:~/distributed_prng_analysis/scorer_trial_results/trial_0002.json \
   │  │      /tmp/trial_0002.json
   │  ├─ Reads JSON from temp file
   │  ├─ Deletes remote file:
   │  │  ssh 192.168.3.120 "rm ~/distributed_prng_analysis/scorer_trial_results/trial_0002.json"
   │  └─ Deletes temp file
   │
   └─ Aggregates all results into single list:
...
┌────────────────────────────────────────────────────────────────┐
│ PHASE 4: OPTUNA UPDATE (report results to LOCAL study)         │
└────────────────────────────────────────────────────────────────┘
...
├── coordinator.py                    # Distributes jobs, pulls results
...
✅ **No Shared Storage:** Each node uses local filesystem only
✅ **Fault Tolerant:** If a node fails, others continue independently
✅ **Simple Setup:** No NFS mounts, no permissions headaches
✅ **Secure:** SSH/SCP for all remote access
✅ **Scalable:** Easy to add/remove nodes
✅ **Debuggable:** Result files preserved until confirmed transferred
✅ **Atomic:** Each trial is independent, no race conditions
...
python3 window_optimizer.py --strategy bayesian --lottery-file synthetic_lottery.json 
    --trials 50 --max-seeds 10000000 --prng-type java_lcg --test-both-modes
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│  window_optimizer.py (Main Entry Point)                         │
├─────────────────────────────────────────────────────────────────┤
│  1. Parse arguments                                              │
│     ├─ --prng-type java_lcg                                     │
│     └─ --test-both-modes True                                   │
│  2. Initialize MultiGPUCoordinator (26 GPUs)                     │
│  3. Create BayesianOptimization instance                         │
│  4. Call coordinator.optimize_window()                           │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│  window_optimizer_integration_final.py                           │
│  optimize_window() method                                        │
├─────────────────────────────────────────────────────────────────┤
│  FOR EACH TRIAL (50 iterations):                                │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  1. Optuna samples next window parameters:                │  │
│  │     ├─ window_size: 512-4096                              │  │
│  │     ├─ offset: 0-1000                                     │  │
│  │     ├─ skip_min: 0-100                                    │  │
│  │     ├─ skip_max: skip_min+50 to 200                       │  │
│  │     └─ sessions: ['midday'] or ['evening'] or both        │  │
│  │                                                            │  │
│  │  2. Call run_bidirectional_test() with these params       │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  run_bidirectional_test()                                 │  │
│  │  ┌─────────────────────────────────────────────────────┐  │  │
│  │  │  PART 1: CONSTANT SKIP MODE                         │  │  │
│  │  │  ├─ Forward sieve: java_lcg                         │  │  │
│  │  │  │  └─ Coordinator distributes across 26 GPUs       │  │  │
│  │  │  │     └─ Each GPU runs sieve_filter.py             │  │  │
│  │  │  │        └─ Finds seeds matching forward pattern   │  │  │
│  │  │  ├─ Reverse sieve: java_lcg                         │  │  │
│  │  │  │  └─ Same process, tests reverse pattern          │  │  │
...
│  │                              ↓                             │  │
│  │  ┌─────────────────────────────────────────────────────┐  │  │
│  │  │  PART 2: VARIABLE SKIP MODE (if test_both_modes)   │  │  │
│  │  │  ├─ Forward sieve: java_lcg_hybrid                  │  │  │
│  │  │  │  └─ Tests variable skip patterns                 │  │  │
│  │  │  ├─ Reverse sieve: java_lcg_hybrid                  │  │  │
│  │  │  ├─ Intersection: forward ∩ reverse                 │  │  │
│  │  │  └─ Tag survivors:                                   │  │  │
│  │  │     ├─ skip_mode: "variable"                        │  │  │
...
│  │  └─────────────────────────────────────────────────────┘  │  │
│  │                              ↓                             │  │
│  │  3. Return TestResult with bidirectional count            │  │
│  └───────────────────────────────────────────────────────────┘  │
│                              ↓                                   │
│  4. Optuna evaluates: score = bidirectional_count                │
│  5. Optuna selects next parameters (TPE algorithm)              │
│  6. Repeat for all 50 trials                                    │
│                              ↓                                   │
│  7. Save all accumulated survivors to:                          │
│     ├─ bidirectional_survivors.json (ALL survivors, tagged)     │
│     ├─ forward_survivors.json                                   │
│     └─ reverse_survivors.json                                   │
│                              ↓                                   │
│  8. Report skip mode distribution:                              │
│     "Constant skip: 6,547 survivors (50.1%)"                    │
│     "Variable skip: 6,546 survivors (49.9%)"                    │
└─────────────────────────────────────────────────────────────────┘
```
...
**Outputs:**
- ✅ `optimal_window_config.json` - Best window parameters
- ✅ `bidirectional_survivors.json` - Survivors with **skip_mode metadata** 🆕
- ✅ `forward_survivors.json` - Forward-only survivors
- ✅ `reverse_survivors.json` - Reverse-only survivors
- ✅ `train_history.json` - 80% of lottery data for training
- ✅ `holdout_history.json` - 20% of lottery data for validation
...
**What it does**: Uses distributed Optuna to find optimal parameters for survivor scoring. **NOW extracts skip_mode as an ML feature!** 🆕
...
**Outputs:**
- ✅ `optimal_scorer_config.json` - Best scorer parameters
...
**What it does**: Scores all survivors using the optimal scorer parameters found in Step 2.5. **Extracts 64 features (was 56)** including skip_mode features! 🆕
...
**Outputs:**
- ✅ `survivors_with_scores.json` - All survivors with quality scores + **64-feature vectors** 🆕
...
**Outputs:**
- ✅ `reinforcement_engine_config.json` - Optimal training configuration
...
**What it does**: Trains the final prediction model using K-Fold cross-validation to prevent overfitting. **Learns skip_mode importance!** 🆕
...
**Outputs:**
- ✅ `models/anti_overfit/best_model.pth` - Final trained model (understands skip_mode)
...
#### NEW: Test Both Modes (⚡ Recommended)
```bash
python3 complete_whitepaper_workflow_with_meta_optimizer.py \
    --lottery-file synthetic_lottery.json \
    --window-opt-trials 50 \
...
| `--prng-type` | `java_lcg` | 🆕 Base PRNG type (java_lcg, mt19937, xorshift32, etc.) |
| `--test-both-modes` | `False` | 🆕 Test BOTH constant AND variable skip |
...
✅ **Prerequisite checking** - Verifies all required files exist
✅ **Output validation** - Confirms each step produced expected files
✅ **Time tracking** - Reports total execution time
✅ 🆕 **Skip mode distribution reporting** - Shows constant vs variable survivor counts
...
| Field | Description |
|-------|-------------|
| `skip_mode` | 🆕 "constant" or "variable" |
| `prng_type` | 🆕 Full PRNG name (e.g., "java_lcg_hybrid") |
| `prng_base` | 🆕 Base PRNG without suffix (e.g., "java_lcg") |
...
Step 1 (Window Optimizer)
  ...
  Outputs: optimal_window_config.json ────┐
           bidirectional_survivors.json ──┼──► Step 2.5
           forward_survivors.json          │
           reverse_survivors.json          │
           train_history.json ────────────┤
...
Step 2.5 (Scorer Meta-Optimizer)          │
  Inputs:  bidirectional_survivors.json ◄─┤
           train_history.json ◄───────────┘
...
### Key Takeaways
- 🆕 **2x sieves** (both modes) = **~1.7x execution time** (not 2x due to Optuna efficiency)
- 🆕 **2x survivors** (both modes) = **~2x file sizes**
- **No increase** in GPU memory per sieve
- **GPU utilization**: 26 GPUs × ~85% = ~22 effective GPUs working simultaneously

---

## What ML Can Learn from Skip Mode 🆕
...
3. skip_mode_variable (0.15) ← NEW! High importance
4. window_x_skip_mode (0.12) ← NEW! Interaction effect
5. skip_entropy (0.09)
6. prng_java_lcg (0.08) ← NEW!
...
```
...
**Symptoms:**
```
⚠️  Warning: window_optimizer_integration_final.py import failed: 
...
### Issue: Zero values in result files (FIXED ✅)
**Symptoms:**
```json
"window_size": 0,
"window_offset": 0,
...
- 🔍 **Debugging** - Need to inspect outputs between steps
- 🧪 **Testing** - Want to verify a specific step works correctly
- 🔧 **Development** - Making changes to individual components
- 📊 **Analysis** - Need to examine intermediate results
- ⚡ **Quick iterations** - Only need to re-run one step
- 🆕 **Comparing PRNGs** - Testing different PRNG types separately

### Use Automated Method When:
- 🚀 **Production runs** - Want to run the complete pipeline
- 🌙 **Overnight jobs** - Set it and forget it
- 📈 **Reproducibility** - Ensure consistent execution order
- 💾 **Logging** - Want complete execution logs
- 🎯 **Final results** - Ready to generate predictions
- 🆕 **Full evaluation** - Testing both constant and variable skip modes
...
- ✅ Backward compatible
- ✅ Production-ready
- ✅ Bug-free (zero values bug fixed)
- ✅ Enhanced with skip mode support
...
pythonif hasattr(args, 'jobs_file') and args.jobs_file:
    use_parallel_dynamic = True  # ← BUG: Launches 26 threads simultaneously
Fixed Code (Line 1545):
pythonif hasattr(args, 'jobs_file') and args.jobs_file:
    use_parallel_dynamic = False  # ✅ Uses ThreadPoolExecutor
```
...
| **ThreadPoolExecutor** | Uses thread pool with max_workers=26 | Internal queue management | Controlled connection ramp-up | Smooth execution ✅ |

**ThreadPoolExecutor Benefits:**
- ✅ Internal queue prevents connection spam
- ✅ Gradual connection ramp-up (not all at once)
- ✅ Respects SSH server limits (MaxStartups)
...
**Why?**
- ✅ No network filesystem (NFS) required
- ✅ No database connection overhead from 26 workers
- ✅ Simple, robust, fault-tolerant
- ✅ Each node completely independent
...
```
1. Workers execute on remotes → Write to LOCAL filesystem
2. Coordinator waits for all jobs to complete
3. Coordinator SSH/SCPs to each node → Pulls result files
4. Coordinator aggregates → Reports to Optuna on zeus
5. Coordinator finds best trial → Saves optimal_scorer_config.json
...
Round-robin assignment: Job 0→zeus GPU0, Job 1→zeus GPU1, Job 2→rig-6600 GPU0, etc.
...
|-----------|--------|------|--------------|-------|
| 100 | 100 | ~20 min | 100/100 ✅ | Production ready |
| 164,105 | 100 | ~35 min | 73/100 ❌ | Before fix (SSH overload) |
| 164,105 | 100 | ~40 min | ~95/100 ✅ | After fix (some retries) |
...
**Solution:** 
✅ **ALREADY FIXED** - Line 1545 of coordinator.py now uses `use_parallel_dynamic = False`

### Issue: Remote jobs failing on first attempt

**Symptom:**
```
⚠️ 192.168.3.120 GPU3 | scorer_trial_5 | 44.0s | Failed (will retry 1/3)
```
...
**Symptom:**
```
✅ localhost GPU0 | scorer_trial_0 | 14.1s
✅ localhost GPU1 | scorer_trial_1 | 24.8s
❌ All remote jobs failing
Possible Causes:
...
```
optuna_studies/
└── scorer_meta_opt_1763949481.db         # Optuna study database

scorer_trial_results/
├── trial_0000.json                        # Individual trial results
├── trial_0001.json
└── ...
...
results/
└── scorer_jobs_results_1763949516.json   # Aggregated results
...
✅ ThreadPoolExecutor for controlled SSH concurrency
✅ PULL architecture (no NFS required)
✅ Automatic fault tolerance and retry logic
✅ 26-GPU distributed execution
✅ Proven working across all nodes
✅ ~20-40 minutes for 100 trials with 164K survivors

Key Takeaway: The fix to use ThreadPoolExecutor instead of Parallel Dynamic mode eliminates SSH connection overload and enables smooth distributed execution across all 26 GPUs! 🎉
...
| **Training 742K seeds** | ~15 min | N/A (use sampling) | - |
| **Training 25K seeds** | ~120s | ~60s | **2x faster** |
...
**Balanced trial:**
```json
{"sample_size": 50000, "batch_size": 256, "epochs": 25}
```
- Total time: ~150s per trial
- Good balance for meta-optimization
...
#### Mining Rig Optimization

**Problem Solved:**
- Mining rigs have 1x PCIe lanes (not 16x)
- Legacy CPU scoring caused severe bottleneck
- Continuous CPU↔GPU transfers saturated bus

**Solution:**
- GPU-vectorized scoring eliminates continuous transfers
- All data loaded once, all computation on GPU
- Result: **Mining rigs now perform identically to high-end workstations**

**Performance on RX 6600 (1x PCIe):**
- Legacy: 180s (PCIe bottlenecked)
- Vectorized: 3.5s (no bottleneck!)
- **51x speedup achieved**
