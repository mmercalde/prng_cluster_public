...
class AdvancedResearch:
    ...
    def gap_aware_reconstruction(self):
        ...
        try:
            ...
            if save_choice:
                ...
                # NEW FORMAT: Use results_manager
                try:
    ...
    def historical_pattern_analysis(self):
        ...
        try:
            top_n = input("Top N most frequent numbers (default: 10): ").strip()
            top_n = int(top_n) if top_n else 10
            
            bottom_n = input("Bottom N least frequent numbers (default: 10): ").strip()
            bottom_n = int(bottom_n) if bottom_n else 10
            
            streak_threshold = input("Minimum streak length to report (default: 2): ").strip()
            streak_threshold = int(streak_threshold) if streak_threshold else 2
            
            # Analysis options
            print("\nEnable/Disable Analysis Modules (y/n):")
            enable_frequency = input("Frequency analysis (default: y): ").strip().lower() != 'n'
            enable_gaps = input("Gap analysis (default: y): ").strip().lower() != 'n'
            enable_streaks = input("Streak analysis (default: y): ").strip().lower() != 'n'
            enable_temporal = input("Temporal patterns (default: y): ").strip().lower() != 'n'
            ...
            bottom_n = 10
            streak_threshold = 2
            enable_frequency = enable_gaps = enable_streaks = enable_temporal = True

        # Get output file
        timestamp = int(time.time())
        default_output = f"results/summaries/historical_analysis_{timestamp}_summary.txt"
        ...
        try:
            ...
            search_id = self.search_manager.create_historical_analysis(data_file, output_file, config)
    ...
    def export_search_results(self):
        ...
        timestamp = int(time.time())
        # NEW FORMAT: Use organized directory structure
        default_file = f"results/json/search_results_{search_id}_{timestamp}.json"
