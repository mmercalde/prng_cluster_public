...
class GPUReverseSieve:
    ...
    def run_reverse_sieve(self,
        ...
        with self.device:
            ...
            best_skips = cp.zeros(n_candidates, dtype=cp.uint8)
            ...
            kernel(
                ...
                (candidate_seeds_gpu, residues, survivors, match_rates, best_skips,
                 survivor_count, cp.int32(n_candidates), cp.int32(k),
                 cp.int32(skip_range[0]), cp.int32(skip_range[1]),
            ...
            if count > 0:
                ...
                skips_cpu = best_skips[:count].get()
