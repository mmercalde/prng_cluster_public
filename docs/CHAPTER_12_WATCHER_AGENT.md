...
### 6.4 Step 5 Manifest: Multi-Model Comparison (Session 72)

**Updated Feb 8, 2026** — Enable `--compare-models` for automatic model selection.

The `reinforcement.json` manifest was updated to enable multi-model comparison:
...
**Why this matters:**
- Step 5 should automatically find the best-performing model
- Without `compare_models`, only one model type trains
- The design intent was always multi-model comparison

**WATCHER command generated:**
````bash
...
**Related:** See Section 6 in Chapter 9 (GPU Process Isolation) for subprocess requirements.
