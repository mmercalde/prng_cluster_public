...
| 3 | PWC ready gate `min_workers=24` | **per GPU** | **YES**, `RuntimeError` | bare-metal `.120/.154/.162` |
...
| CLI, `--use-persistent-workers` | **YES, per GPU** — `min_workers` default **24** = "full 3-rig AMD cluster" (`window_optimizer.py:1323`), hard `RuntimeError` before dispatch (`persistent_worker_coordinator.py:884-901`) | **tightened** |
...
own help text as "full 3-rig AMD cluster"). For the legacy and PWC paths **P0.5
