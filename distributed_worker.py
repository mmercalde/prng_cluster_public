#!/usr/bin/env python3
"""
distributed_worker.py - Multi-GPU Distributed PRNG Worker
VERSION: 1.8.0 - Fixed script job GPU initialization conflict

CHANGELOG:
- v1.8.0: CRITICAL FIX - Restored `if analysis_type != 'script':` condition for GPU init.
          Script jobs (scorer_trial_worker.py) handle their own PyTorch GPU setup.
          Parent process CuPy init was conflicting with subprocess PyTorch CUDA context.
          This fix allows localhost script jobs to work alongside remote execution.
"""
...
_cupy = None
# Global thread pool executor for script-based jobs
...
def execute_analysis_job(job_data: Dict[str, Any], args) -> Dict[str, Any]:
    ...
    try:
        ...
        # Pre-run guard (only for non-script jobs - script jobs handle their own GPU)
        ...
        if analysis_type == 'sieve':
            ...
            # These jobs handle their own GPU initialization via PyTorch
            # DO NOT initialize CuPy/CUDA here - it will conflict with subprocess
            import subprocess
            import json
            import os
            print(f"[analysis] Using script-based execution (subprocess handles GPU)")
            ...
            if not script:
                ...
                def execute_script_threaded():
                    """Execute script in a worker thread so GPU can overlap other work."""
                    try:
                ...
                # Submit to the global thread pool and wait for this job's result
                future = _executor.submit(execute_script_threaded)
        ...
        # Post-run quick guard to detect backend drift (skip for script jobs)
        if analysis_type != 'script':
            enforce_gpu_backend(job_data, args.gpu_id)
...
def main():
    ...
    # ==========================================================================
    # CRITICAL: Skip GPU init for script jobs!
    # Script jobs (scorer_trial_worker.py) handle their own PyTorch GPU setup.
    # If we initialize CuPy/CUDA here, it creates a context conflict when the
    # subprocess tries to init its own PyTorch CUDA context.
    # 
    # This is why localhost script jobs failed with:
    ...
    if analysis_type != 'script':
        ...
        print(f"[script] Skipping GPU init - subprocess will handle its own CUDA context")
    ...
    try:
        ...
        # Check for success - scorer_trial_worker uses "status": "success", others use "success": true
