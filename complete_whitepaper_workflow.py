#!/usr/bin/env python3
"""
Complete Whitepaper Workflow Orchestrator - (v3.0 - Corrected 6-Step)
======================================================================
...
def main(args):
    ...
    print("="*70)
    print("COMPLETE WHITEPAPER WORKFLOW (v3.0 - Corrected 6-Step)")
    print("="*70)
    print("\nThis orchestrator will execute the full, distributed pipeline:")
    ...
    # List of all required scripts and configs
    required_files = [
        # Step 1
        ...
        'run_ml_distributed.sh', # This is often called by Step 4's script
        'anti_overfit_trial_worker.py', # Worker for Step 4
    ...
    if not run_command(
        ['python3', 'window_optimizer_bayesian.py',
         '--lottery-file', args.lottery_file,
         '--trials', str(args.window_opt_trials),
         '--output-config', optimal_window_config
        ],
    ...
    if not Path(optimal_window_config).exists():
        ...
        print(f"\n✅ Optimal sieve config saved to {optimal_window_config}")
    ...
    if not Path(survivor_file).exists():
        print(f"\n❌ {survivor_file} was not created by Step 2.")
        return 1
    ...
    print("This will use: run_scorer_meta_optimizer.sh")
    ...
    if not Path(optimal_scorer_config).exists():
        print(f"\n❌ {optimal_scorer_config} was not created by Step 3.")
        print("This file is critical for the next steps. Aborting.")
        return 1
    else:
    ...
    if not run_command(
        ['python3', 'adaptive_meta_optimizer.py',
         ...
         '--survivor-data', survivor_file,
         '--apply'], # <-- This applies settings to reinforcement_engine_config.json
    ...
    print("This will use the pruned version of meta_prediction_optimizer_anti_overfit.py")
    ...
    if not run_command(
        ['python3', 'meta_prediction_optimizer_anti_overfit.py',
         ...
         '--distributed'], # <-- Flag to run distributed
    ...
    try:
        ...
        with open(holdout_history_file, 'r') as f:
            # Handle potential empty file or different structure
            data = json.load(f)
            if isinstance(data, list) and len(data) > 0 and isinstance(data[0], dict):
                holdout_history = [d['draw'] for d in data]
            else:
                holdout_history = data # Assume list of ints
        ...
        # 1. Init engine (loads default configs)
        engine = ReinforcementEngine(
            config=ReinforcementConfig(), # Loads reinforcement_engine_config.json
            lottery_history=holdout_history
        )
        
        # 2. Load the final trained model weights from Step 5
        engine.load_model(final_model_path)
        print("✅ Final model loaded successfully.")

        # 3. Generate test survivors
        import random
    ...
    print("="*70)
    print("\nWorkflow Summary:")
    print(f"  1. ✅ Meta Window Optimizer: Ran {args.window_opt_trials} trials.")
    print(f"  2. ✅ Sieves: Ran successfully with optimal params.")
    print(f"  3. ✅ Scorer Meta-Optimizer: Ran {args.scorer_trials} distributed trials.")
    print(f"  4. ✅ Adaptive Optimizer: Ran successfully.")
    print(f"  5. ✅ Anti-Overfit Optimizer: Ran {args.anti_overfit_trials} distributed trials.")
    print(f"  6. ✅ Final Prediction: Ran successfully.")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Complete Whitepaper Workflow Orchestrator (v3.0 - 6-Step)')
    ...
    # Set environment variable to confirm distributed mode,
    # in case any scripts check it.
